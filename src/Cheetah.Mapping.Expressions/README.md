...
- **Коллекции:** Маппинг `List<T>`, массивов, `IEnumerable<T>`/`IReadOnlyList<T>` с поэлементной конвертацией.
- **Маппинг через конструктор:** Если у типа-назначения нет беспараметрического конструктора (позиционные `record`'ы, CQRS-команды/запросы), параметры конструктора сопоставляются со свойствами источника по имени (без учёта регистра).
- **Поддержка Nullable типов:** Автоматическая конвертация `T` <-> `Nullable<T>`.
- **Встроенные скалярные конверсии:** `Guid <-> string` (включая `Guid?`).
- **In-place маппинг:** `Map(source, destination)` копирует свойства в существующий объект.

## 🧩 Точки расширения

Маппер не использует профили/Fluent-конфигурацию, но позволяет регистрировать конвертеры типов и помечать
типы-назначения, требующие особой сборки. Регистрация — глобальная (статический `ExpressionBuilder`),
делается один раз на старте (обычно в `ConfigureServices` модуля-расширения) и сбрасывает кэш скомпилированных мапперов.

```csharp
// Конвертер значения source -> dest
ExpressionBuilder.RegisterConverter<DateTime, Timestamp>(
    d => Timestamp.FromDateTime(DateTime.SpecifyKind(d, DateTimeKind.Utc)));

// Типы, которые нужно собирать "null-safe": ссылочные поля не присваиваются null
...
> **Поддержка proto/gRPC** реализована как opt-in расширение в модуле **`Cheetah.Mapping.Expressions.Protobuf`**
> (`CrmCustomMappingProtobufModule`) — он регистрирует конверсии `Timestamp` и предикат `IMessage`.
> Само ядро маппера от `Google.Protobuf` НЕ зависит.
...
- Нет Fluent API для пер-полевой конфигурации (игнор/переименование на лету). Пер-полевое поведение —
  только через атрибуты `[MapIgnore]` / `[MapProperty("...")]` на типе-назначении; конверсии — на уровне типов через `RegisterConverter`.
- Конфигурация конвертеров/предикатов глобальная и статическая (см. «Точки расширения»): регистрируйте на старте, до первых маппингов.
