...
```
{Prefix}.{VacancyTasks}/
├── {Prefix}.{VacancyTasks}.DomainEvents/     # События домена
├── {Prefix}.{VacancyTasks}.Domain/           # Сущности, интерфейсы репозиториев
├── {Prefix}.{VacancyTasks}.Contracts/        # DTO, Requests, ViewModels
├── {Prefix}.{VacancyTasks}.DataAccess/       # DbContext, конфигурации EF Core, репозитории
├── {Prefix}.{VacancyTasks}.Application/      # Commands, Queries, Handlers (CQRS)
├── {Prefix}.{VacancyTasks}.ApiClient/        # HTTP клиент для интеграции
├── {Prefix}.{VacancyTasks}.Api/              # Minimal API endpoints
├── {Prefix}.{VacancyTasks}.Domain.Tests/     # Unit тесты Domain
├── {Prefix}.{VacancyTasks}.Application.Tests/# Unit тесты Application
├── {Prefix}.{VacancyTasks}.ApiClient.Tests/  # Unit тесты ApiClient
└── {Prefix}.{VacancyTasks}.Api.Tests/        # Integration тесты API
```
...
```bash
dotnet sln Cheetah.slnx add src/Modules/{VacancyTasks}/**/*.csproj
```
...
{
  "ConnectionStrings": {
    "{VacancyTasks}": "Host=localhost;Database=cheetah;Username=postgres;Password=postgres"
  }
}
```
...
```bash
dotnet ef migrations add Initial -p src/Modules/{VacancyTasks}/{Prefix}.{VacancyTasks}.DataAccess -s src/Hosts/Cheetah.Admin.Api
```
...
```csharp
[DependsOn(typeof({VacancyTasks}BootstrapperModule))]
...
- **{VacancyTasks}DbContext.cs** - DbContext с конфигурацией
...
- **I{VacancyTasks}Service.cs** - Интерфейс HTTP клиента
- **{VacancyTasks}Service.cs** - Реализация HTTP клиента
...
dotnet test src/Modules/{VacancyTasks}/

# Только unit тесты
dotnet test src/Modules/{VacancyTasks}/{Prefix}.{VacancyTasks}.Domain.Tests/
dotnet test src/Modules/{VacancyTasks}/{Prefix}.{VacancyTasks}.Application.Tests/

# Integration тесты (требуют Docker для PostgreSQL)
dotnet test src/Modules/{VacancyTasks}/{Prefix}.{VacancyTasks}.Api.Tests/
```
