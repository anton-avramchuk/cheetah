...
/// <summary>Заметка закреплена на карточке сущности (влияет на порядок выдачи).</summary>
public record NotePinnedIntegrationEvent(Guid NoteId, string EntityType, Guid EntityId) : EventBase;

/// <summary>Заметка откреплена.</summary>
public record NoteUnpinnedIntegrationEvent(Guid NoteId, string EntityType, Guid EntityId) : EventBase;
