...
public sealed record CreateTeamMemberCommand(string Name, Guid? UserId) : ICommand<Guid>;
...
public sealed class CreateTeamMemberCommandHandler : ICommandHandler<CreateTeamMemberCommand, Guid>
{
    ...
    {
        var member = TeamMember.Create(command.Name, command.UserId);
...
/// <summary>Обновить участника (имя и привязку к пользователю).</summary>
public sealed record UpdateTeamMemberCommand(Guid Id, string Name, Guid? UserId) : ICommand;
...
public sealed class UpdateTeamMemberCommandHandler : ICommandHandler<UpdateTeamMemberCommand>
{
    ...
    public async ValueTask HandleAsync(UpdateTeamMemberCommand command, CancellationToken ct = default)
    {
        ...
        member.LinkUser(command.UserId);
        await _repository.SaveChangesAsync(ct);
    }
}
