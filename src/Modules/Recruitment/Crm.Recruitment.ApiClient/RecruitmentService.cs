...
public class RecruitmentService : IRecruitmentService
{
    ...
    public RecruitmentService(HttpClient httpClient)
    {
        _httpClient = httpClient;
    }

    public async ValueTask<IReadOnlyList<VacancyViewModel>> GetAllAsync(CancellationToken ct = default)
    {
        var response = await _httpClient.GetAsync(BasePath, ct);
        response.EnsureSuccessStatusCode();

        var entities = await response.Content.ReadFromJsonAsync<List<VacancyViewModel>>(ct);
        return entities ?? [];
    }

    public async ValueTask<VacancyViewModel?> GetByIdAsync(Guid id, CancellationToken ct = default)
    {
        ...
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<VacancyViewModel>(ct);
    }

    public async ValueTask<Guid> CreateAsync(CreateVacancyRequest request, CancellationToken ct = default)
    {
    ...
    public async ValueTask UpdateAsync(Guid id, UpdateVacancyRequest request, CancellationToken ct = default)
    {
