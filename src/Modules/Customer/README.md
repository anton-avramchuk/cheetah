...
```
{Prefix}.{Customer}/
├── {Prefix}.{Customer}.DomainEvents/     # События домена
├── {Prefix}.{Customer}.Domain/           # Сущности, интерфейсы репозиториев
├── {Prefix}.{Customer}.Contracts/        # DTO, Requests, ViewModels
├── {Prefix}.{Customer}.DataAccess/       # DbContext, конфигурации EF Core, репозитории
├── {Prefix}.{Customer}.Application/      # Commands, Queries, Handlers (CQRS)
├── {Prefix}.{Customer}.ApiClient/        # HTTP клиент для интеграции
├── {Prefix}.{Customer}.Api/              # Minimal API endpoints
├── {Prefix}.{Customer}.Domain.Tests/     # Unit тесты Domain
├── {Prefix}.{Customer}.Application.Tests/# Unit тесты Application
├── {Prefix}.{Customer}.ApiClient.Tests/  # Unit тесты ApiClient
└── {Prefix}.{Customer}.Api.Tests/        # Integration тесты API
```
...
```bash
dotnet sln Cheetah.slnx add src/Modules/{Customer}/**/*.csproj
```
...
{
  "ConnectionStrings": {
    "{Customer}": "Host=localhost;Database=cheetah;Username=postgres;Password=postgres"
  }
}
```
...
```bash
dotnet ef migrations add Initial -p src/Modules/{Customer}/{Prefix}.{Customer}.DataAccess -s src/Hosts/Cheetah.Admin.Api
```
...
```csharp
[DependsOn(typeof({Customer}BootstrapperModule))]
...
- **{Customer}DbContext.cs** - DbContext с конфигурацией
...
- **I{Customer}Service.cs** - Интерфейс HTTP клиента
- **{Customer}Service.cs** - Реализация HTTP клиента
...
dotnet test src/Modules/{Customer}/

# Только unit тесты
dotnet test src/Modules/{Customer}/{Prefix}.{Customer}.Domain.Tests/
dotnet test src/Modules/{Customer}/{Prefix}.{Customer}.Application.Tests/

# Integration тесты (требуют Docker для PostgreSQL)
dotnet test src/Modules/{Customer}/{Prefix}.{Customer}.Api.Tests/
```
