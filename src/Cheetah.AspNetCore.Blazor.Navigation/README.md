...
Видимость по фич-флагам — второй шов, `IFeatureVisibilityEvaluator` (`IsEnabledAsync(feature)`) из `Cheetah.AspNetCore.Blazor.Abstractions`, реализуемый приложением поверх `IFeatureManager`; Navigation так же не зависит от FeatureManagement. По умолчанию зарегистрирован `NullFeatureVisibilityEvaluator` (все фичи включены), поэтому хост без движка флагов работает как раньше. Пункт прячется через `.RequireFeature("Vacancy.Teams")`.
...
> Меню — это не защита: скрытый пункт не мешает открыть URL напрямую. Страницу гейтите `[RequireFeature]` + `FeatureGate` (см. `Cheetah.AspNetCore.Blazor.Layouts`).
...
- Шов `IFeatureVisibilityEvaluator` (в `Blazor.Abstractions`) реализуется приложением поверх `IFeatureManager`; дефолт — `NullFeatureVisibilityEvaluator` через `TryAdd`, все фичи включены.
